...
const move_1 = __nccwpck_require__(624);
async function run() {
    try {
        const config = {
        ...
        const transitions = JSON.parse(core.getInput("transitions"));
        console.debug(`Given transitions: ${JSON.stringify(transitions)}`);
        await move_1.move(transitions, config);
    }
    catch (error) {
...
const perform = async (issueId, dstId, config) => {
    var _a, _b;
    const body = { transition: { id: dstId } };
    await jira_1.sendRequest(`/rest/api/3/issue/${issueId}/transitions`, config, {
        method: "POST",
        body,
    });
    const resp = await jira_1.sendRequest(`/rest/api/2/issue/${issueId}`, config);
    if (resp.status === 200) {
        const issue = JSON.parse(await resp.text());
        const status = (_b = (_a = issue.fields) === null || _a === void 0 ? void 0 : _a.status) === null || _b === void 0 ? void 0 : _b.name;
        if (status) {
...
const move = async (transitions, config) => {
    await Promise.all(Object.entries(transitions).map(([issueId, dstId]) => perform(issueId, dstId, config)));
};
exports.move = move;
